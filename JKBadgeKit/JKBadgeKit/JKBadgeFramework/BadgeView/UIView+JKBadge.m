...
#define kJkBadgeDefaultFont                ([UIFont boldSystemFontOfSize:9])
#define kJkBadgeDefaultMaximumBadgeNumber  99
static const CGFloat kJKBadgeDefaultRadius = 3.f;
@implementation UIView (JKBadge)
- (UILabel *)badge{
    UILabel *bLabel   = objc_getAssociatedObject(self, _cmd);
    if (!bLabel) {
        CGFloat width = kJKBadgeDefaultRadius * 2;
        ...
        bLabel.layer.cornerRadius  = kJKBadgeDefaultRadius;
...
- (UIFont *)badgeFont {
    return objc_getAssociatedObject(self, _cmd) ?:kJkBadgeDefaultFont;
}
...
- (void)showBadgeWithValue:(NSUInteger)value{
    ...
    self.badge.text   = (value > kJkBadgeDefaultMaximumBadgeNumber ?
                         [NSString stringWithFormat:@"%@+", @(kJkBadgeDefaultMaximumBadgeNumber)] :
